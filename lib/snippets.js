...
module.exports = {
  ...
  add (filePath, snippetsBySelector, packageName = null, isDisabled = false) {
    for (const selector in snippetsBySelector) {
      ...
      const unparsedSnippetsWithoutPrefixByCommandName = {}
      for (const name in snippetsByName) {
        ...
        if (command) {
          if (!isValidCommandName(command)) {
            showInvalidCommandNameNotification(name, commandName)
            continue
          }
          if (!prefix) {
  ...
  addCommandForSnippet (attributes, packageName, selector) {
    ...
    if (CommandMonitor.exists(commandName)) {
      ...
      showCommandNameConflictNotification(name, commandName, packageName, this.getUserSnippetsPath())
      // No need to remove the snippet altogether, but let's null out the
      // `command` property to prevent any possible confusion.
      attributes.command = null
      return
    }

    let commandHandler = (event) => {
      let editor = event.target.getModel()
    ...
    let disposable = atom.commands.add(
      'atom-text-editor',
      `${packageName}:${attributes.command}`,
      commandHandler
    )
  ...
  snippetForPrefix (snippets, prefix, wordPrefix) {
    ...
    for (const snippetPrefix in snippets) {
      // Any snippet without a prefix was keyed on its command name, but with a
  ...
  insertSnippetFromCommand (snippet, editor = null, cursor = null) {
    if (!editor) { editor = atom.workspace.getActiveTextEditor() }
    if (!cursor) { editor.getLastCursor() }

    return this.insert(snippet, editor, cursor)
  },
