...
function makeDateResolver (params) {
  let locale = remote.app.getLocale()
  return () => new Date().toLocaleString(locale, params)
}
...
const RESOLVERS = {
  ...
  'WORKSPACE_FOLDER' ({editor}) {
    // Like $RELATIVE_FILEPATH, but in reverse: find whichever project
    // directory is the ancestor of the file, then return just that directory.
    let projectPaths = atom.project.getPaths()
    if (projectPaths.length === 0) { return '' }
    let filePath = editor.getPath()
    let ancestor = projectPaths.find(pp => {
  ...
  // UUID
  //
  // (can be done without dependencies once we use Node >= 14.17.0 or >=15.6.0)
  //
}
